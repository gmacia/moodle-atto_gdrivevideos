...
var CSS = {
        ...
        KEYBUTTON: 'atto_gdrivevideos_templatebutton',
        HEADERTEXT: 'atto_gdrivevideos_headertext',
        INSTRUCTIONSTEXT: 'atto_gdrivevideos_instructionstext',
        TEMPLATEVARIABLE: 'atto_gdrivevideos_templatevariable',
...
var BUTTONTEMPLATE = '' +
        '<div id="{{elementid}}_{{innerform}}" class="atto_gdrivevideos_buttons mdl-align">' +
            '<button class="' + CSS.KEYBUTTON + '_{{templateindex}}">{{name}}</button>' +
        '</div>';
		
var FIELDTEMPLATE = '' +
        '<div id="{{elementid}}_{{innerform}}" class="mdl-align">{{variable}}' +
            '&nbsp;<input type="text" class="' + CSS.TEMPLATEVARIABLE + '_{{variableindex}} atto_gdrivevideos_field" value="{{defaultvalue}}"></input>' +
        '</div>';
var SELECTCONTAINERTEMPLATE = '' +
            '<div id="{{elementid}}_{{innerform}}" class="mdl-align">{{variable}}</div>';
			
var SELECTTEMPLATE = '' +
            '<select class="' + CSS.TEMPLATEVARIABLE + '_{{variableindex}} atto_gdrivevideos_field"></select>';
...
Y.namespace('M.atto_gdrivevideos').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _displayDialogue: function(e, clickedicon) {
	    ...
	    /**
     * Display the chosen gdrivevideos template form
     *
     * @method _displayDialogue
     * @private
     */
  ...
  /**
   ...
   /**
    ...
    _doInsert : function(e,templateindex){
        ...
        var retstring = "{GDRIVEVIDEOS:type=";
        var thekey = this.get('keys')[templateindex];
        var thename = this.get('names')[templateindex];
        var thevariables=this.get('variables')[templateindex];
        var thedefaults=this.get('defaults')[templateindex];
      ...
      //  var defaultsarray=this._getDefArray(thedefaults);
        ...
        if(theend){
        	retstring += '<br/>{GDRIVEVIDEOS:type="' + thekey + '_end"}';
        }
